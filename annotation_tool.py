...
if X and dics and 'TYPE' in X:
    if X['TYPE'] == 'class':
        ...
        while i < len(dics):
            ...
            st.write('#####################################################')
            start_time = time.time()
            R = st.text_input("Enter your annotation (press 'R' to go back): ", key=f"annotation_input_class_{i}")
        ...
        while i < len(dics):
            ...
            st.write('#####################################################')
            start_time = time.time()
            R = st.text_input("Enter your annotation (press 'R' to go back): ", key=f"annotation_input_num_{i}")
            if R == 'R':
                if i > 0:
                    i -= 1
